...
function SelectPlan() {
  ...
  const cardData: CardItem[] = [
    {image:"/src/public/icon-arcade.svg" , name: "Arcede", price: 90, duration: "2 months free" },
    {image:"/src/public/icon-advanced.svg" , name: "Advanced", price: 120, duration: "2 months free" },
    {image:"/src/public/icon-pro.svg" , name: "Pro", price: 150, duration: "2 months free" },
  ];
  const planType = !toggled ? "Yearly" : "Monthly";

  function handleShooseCard({ card }: { card: string; }): void {
        setScard([card , planType ]);
    } 
...
<label key={index} onClick={() => handleShooseCard({ card })}>
          ...
          <span className="radio-tile flex  flex-col  items-start  p-4 text-[var(--Blue-950)] pr-7">
            ...
            <span className="text-[var(--Grey-500)] text-[.8rem]">${card.price}/yr</span>
      ...
      <div className="p-3 bg-[var(--Blue-100)] rounded-[.4rem] mt-8 flex items-center justify-center w-[80%] gap-6">
        <span
